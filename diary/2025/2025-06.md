...
- Next/Image はデフォルト lazy(遅延)のため、priority で優先度を上げられる。ヒーロー画像など。
- sizes は 10vw などビューポートを指定する。(max-width: 768px) 50vw などメディアクエリのような指定もできる
- fill は親要素全体に広げるときに使う。親要素に relative を指定する。
